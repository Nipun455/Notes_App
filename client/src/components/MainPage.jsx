...
const NOTES_URL=import.meta.env.REACT_APP_BASE_URL;
...
const MainPage = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            const response = await axios.post(`${NOTES_URL}/api/v1/notes/getusernotes`, null, {
                headers: {
    ...
    const saveNotes = async(id,newText) => {
        const token = localStorage.getItem("Token");
        await axios.post(`${NOTES_URL}/api/v1/notes/addnotes`,newText,{
            headers: {
                'Authorization': `Bearer ${token}`
            }
        });
    ...
    const removeNotes = async(_id) => {
        const id={_id:_id};
        const token = localStorage.getItem("Token");
        await axios.post(`${NOTES_URL}/api/v1/notes/remove`,id,{
            headers: {
                'Authorization': `Bearer ${token}`
            }
        });
